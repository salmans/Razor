...
{- SMTContainer contains all the symbolic values and names for addressing them 
   ...
   [@containerResult@] is used to store the result from SMT solver that is 
...
instance SATIterator SMTContainer where
  ...
  satAugment cont   = let (res, cont') = augment cont
                      in  (translateSolution res, cont')
  satBacktrack cont = let (res, cont') = backtrack cont
                      in  (translateSolution res, cont')
...
{- Aguments the given iterator with additional constraints to preserve the
   information in the current model for augmentation. This function assumes that
   the ground sequents that are generated by the Chase during augmentation have
   been previously added to the existing iterator; i.e., the iterator already
   contains the ground sequents that are generated during augmentation. -}
augment :: SMTContainer -> (SatResult, SMTContainer)
augment cont =
  let context = do
            container   <- lift State.get
            assertForce <- case containerResult container of
                             Nothing -> return true
                             Just  r -> forceResult r
            assert assertForce
            push
            next <- getSatResult
            min  <- reduce next
